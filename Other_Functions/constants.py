...
def constants(constant_name):
    ...
    def check(string):
        if string == 'km2DU':
            out = 149597870.700         # [km/DU] (1AU) Curtis pg. 722
            index = -1
        elif string == 's2TU':
            out = 5022643.121           # [s/TU] making \mu_sun = 1
            index = -1
        elif string == 'days2TU':
            out = 58.13244353           # [days/TU]
            index = -1
        elif string == 's2day':
            out= 86400                  # [s/day]
            index = -1
        elif string =='muSun':
            out = 132712428000          # [km^3/s^2] Vallado pg. 1043
            index = -1
        else:
            out = 0
            index = 0
        return out,'NA',index
    [constant_value,constant_units,index] = check(constant_name)
    ...
    if index == 0:
        ...
        values = [  695800,         2440,             6052,          6378,        1737.4,       3390,          69911,           58232,          25362,          24622,           1184,          14      ]
    ...
    if index > -1:
        ...
        # constant_value from check()
        pass 
    else:
        raise Exception('Constant not found. Try a different name.')
        return
    
    return constant_value,constant_units
